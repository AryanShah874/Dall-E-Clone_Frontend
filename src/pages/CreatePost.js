...
function CreatePost(){
    ...
    const handleSubmit=async(event)=>{
        ...
        if(form.prompt && form.photo){
            ...
            try {
                const response=await fetch("https://dall-e-clone-backend-7h7c.onrender.com/api/v1/post", {
                    method: 'POST',
                    headers: {
    ...
    const generateImage=async()=>{
        if(form.prompt){
            try{
                ...
                const response=await fetch("https://dall-e-clone-backend-7h7c.onrender.com/api/v1/dallE", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
